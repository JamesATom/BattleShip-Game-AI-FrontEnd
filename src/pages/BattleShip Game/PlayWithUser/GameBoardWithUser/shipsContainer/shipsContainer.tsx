...
import { SET_SHIP_POSITIONS_MUTATION } from '../../../../../utils/graphqlQuery/gql';
import { useMutation } from '@apollo/client';

export default function ShipContainer(props: any) {
    const [setShipPositions, { error: mutationError }] = useMutation(SET_SHIP_POSITIONS_MUTATION);
    const { 
        ships, orientation, 
        positionSetObject, 
        setSelectedImage, 
    ...
    const handleClick = () => {
        ...
        .then((response) => {
            if (response.data.setShipPositions.statusCode == 102) {
                sessionStorage.setItem('myTurn', 'true');
                setAlertText(response.data.setShipPositions.message);
                setOpen(true);
            } 
        })
        .catch((error) => {
            console.log("Network error in shipsContainer.tsx: ", error);
        });
    }
